...
<div class="card">
    ...
    @include('vendor.backpack.licenses_in_staff.subjects', ['subjects_staff' => \App\Models\StaffSubject::where('staff_id', $entry->id)->where('subject_id', '!=', 0)->get()], ['staff_id' => $entry->id])
    @include('vendor.backpack.licenses_in_staff.licenses', ['licenses' => \App\Models\StaffLicense::where('staff_id', $entry->id)->get()], ['staff_id' => $entry->id])
    @include('vendor.backpack.licenses_in_staff.discounts', ['discounts' => \App\Models\StaffDiscount::where('staff_id', $entry->id)->get()], ['staff_id' => $entry->id])
    </div>
</div>
@else
    @include('crud::inc.show_fields', ['fields' => $crud->getFieldsWithoutATab()])
...
<div class="tab-container {{ $horizontalTabs ? '' : 'container'}} mb-2">
    ...
    <div class="nav-tabs-custom {{ $horizontalTabs ? '' : 'row'}}" id="form_tabs">
        ...
        <div class="tab-content p-0 {{$horizontalTabs ? '' : 'col-md-12'}}">
...
{{--@include('crud::fields.'.$fields['id']['type'], ['field' => $fields['id']])--}}
