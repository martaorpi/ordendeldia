            ...
            <form method="post" action="{{ url('formulario-inscripcion') }}" class="" enctype="multipart/form-data">
                ...
                <div class="form-group row">
                    <div class="col-12 col-lg-6">
                        ...
                        <select class="form-control selectpicker" data-live-search="true" id="departamento" name="departament_id" disabled>
                            <option value="">Seleccione</option>
                            @foreach ($departamentos as $departamento)
                                <option value="{{ $departamento->id }}" {{(old('departament_id')==$departamento->id)? 'selected':''}}>{{ $departamento->description }}</option>
                            @endforeach
                        </select>
                    </div>
        ...
        if(document.getElementById('provincia').value != 0){
            $.get("getDptos/"+document.getElementById('provincia').value,function(response,indicador){
                ...
                if(document.getElementById('departamento').value != 0){
                    $.get("getLocalidades/"+document.getElementById('departamento').value,function(response,indicador){
                        $("#localidad").empty();
                        for(i=0; i<response.length; i++){
